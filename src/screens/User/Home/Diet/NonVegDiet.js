...
const NonVegDiet = ({ route }) => {
  ...
  const NonVegDietPlanApi = async () => {
    ...
    UserAxios.getResponse(
      `${appStrings.screens.api_names.nonveg_dietPlan}${patientid}&type=${type}&day_id=${day_id}&date=${_date}`,
      inputParams,
      "get",
      (response, error) => {
        if (!error) {
    ...
    return (
      <View style={styles.listItem}>
        <TouchableOpacity activeOpacity={0.7}
          onPress={() => navigationRef.current?.navigate(appStrings.screens.appStrings.nonvegDiet)}
        >
